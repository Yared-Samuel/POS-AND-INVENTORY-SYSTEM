...
const SpriceList = () => {
  ...
  const handleSave = (_id, newData) =>{
    // Handle saving the edited data, e.g. update it in your data array
    // and then update the state
    const updatedData = data.map((row)=>
      row._id === _id ? {...row, ...newData} : row
    );
    setData(updatedData)
    handelEdit(_id)
  } 
  ...
  return (
    ...
    <div className='table'>
      ...
      <button className='--btn --btn-primary button-create' onClick={goCreateSprice}>Set Selling Price</button>
        <span>
        <Search 
          type="text"
          value={filterText}
